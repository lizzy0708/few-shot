...
from models.raw_classifier_model import RawClassifierModel
...
def train(args):
    ...
    for domain in args.train_domains:
        datasets.append(HUSTDataset(root=args.root, domain=domain, only_normal=False,
                                     shot=None, all_domains=args.all_domains))
    dataset = ConcatDataset(datasets)
    loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True,
                         num_workers=2, drop_last=False)

    model = RawClassifierModel(num_classes=args.num_classes, num_domains=num_domains,
                                encoder_layer=args.encoder_layer).to(device)
    ...
    print("===================================")
    print("Train RawClassifierModel (no decomposition, GRL-on)")
    print("===================================")
    print("Device        :", device)
    ...
    for epoch in range(args.epochs):
        ...
        total_loss = total_class = total_dom = 0.0
        ...
        for batch in loader:
            ...
            correct_dom += (out["domain_logits"].argmax(dim=1) == domain).sum().item()
