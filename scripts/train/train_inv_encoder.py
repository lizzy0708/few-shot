...
def normal_contrastive_loss(z_inv, labels, temperature=0.07):
    z = F.normalize(z_inv, dim=1)
...
def prototype_alignment_loss(z_inv, labels, domains):
    z = F.normalize(z_inv, dim=1)
    normals = z[labels == 0]
...
def episodic_proto_loss(z_inv, labels, n_support=4, n_episodes=4):
    """4-shot 테스트 시나리오 시뮬레이션 (normalized feature space)."""
    device = z_inv.device
    normal_idx = (labels == 0).nonzero(as_tuple=True)[0]
    if len(normal_idx) < n_support + 1 or not (labels == 1).any():
        return torch.tensor(0.0, device=device)

    z_norm = F.normalize(z_inv, dim=1)
    losses = []
...
def train(args):
    ...
    print("=" * 45)
    print("InvEncoderModel Training")
    print("=" * 45)
    ...
    print(f"LR            : {args.lr}")
    print(f"inv_dim       : {args.inv_dim}")
    print(f"domain_weight : {args.domain_weight}")
    print(f"supcon_weight : {args.supcon_weight}")
    print(f"proto_weight  : {args.proto_weight}")
    print(f"episodic_wt   : {args.episodic_weight}")
    ...
    for epoch in range(args.epochs):
        ...
        total_loss = total_cls = total_dom = total_sc = total_pa = total_ep = 0.0
        ...
        for batch in loader:
            ...
            episodic_loss = episodic_proto_loss(z_inv, binary_label)
        ...
        print(
            ...
            f"Dom={total_dom/n:.4f} | SC={total_sc/n:.4f} | "
            f"PA={total_pa/n:.4f} | Ep={total_ep/n:.4f} | "
...
def main():
    ...
    parser.add_argument("--inv_dim", type=int, default=512)
    ...
    parser.add_argument("--save_path", type=str, default="inv_encoder.pth")
