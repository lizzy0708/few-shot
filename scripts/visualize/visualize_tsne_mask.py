...
def collect_features(args, device):
    ...
    for batch in loader:
        ...
        # autograd.grad 때문에 no_grad 사용 X
        out = model(img)
...
def main():
    ...
    print("Collected features:", features.shape)
    ...
    plot_tsne(
        emb,
        domains,
        title=f"t-SNE of {args.feature} ({mode}) by domain",
        save_path=save_domain,
        legend_title="Domain",
    )

    # Label plot (normal-only면 생략 가능)
    if not args.only_normal:
        ...
        plot_tsne(
            emb,
            label_names,
            title=f"t-SNE of {args.feature} ({mode}) by label",
